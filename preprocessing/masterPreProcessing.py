...
# Print a formatted section banner for each major pipeline stage.
...
# Print one aligned status line for readable progress logging.
def print_step(label: str, value) -> None:
    print(f"  {label:<45} {value}")


# Normalize ZIP values into consistent 5-character zero-padded strings.
def zip_str(series: pd.Series) -> pd.Series:
...
# Load all prerequisite cleaned feature tables used in the master join.
def load_input_tables() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    return realtor, zillow_zip, census_zip, metro


# Standardize ZIP columns across tables before performing ZIP-based merges.
def normalize_zip_codes(
    realtor: pd.DataFrame,
    zillow_zip: pd.DataFrame,
    ...
    return realtor, zillow_zip, census_zip


# Merge ZIP-level Zillow features into the property-level base table.
def join_zillow_zip_features(realtor: pd.DataFrame, zillow_zip: pd.DataFrame) -> tuple[pd.DataFrame, int]:
...
# Merge ZIP-level Census features into the evolving master table.
def join_census_zip_features(master: pd.DataFrame, census_zip: pd.DataFrame, before: int) -> pd.DataFrame:
...
# Aggregate metro features to state level and merge them by state abbreviation.
def join_metro_features(master: pd.DataFrame, metro: pd.DataFrame, before: int) -> pd.DataFrame:
    ...
    state_metro = (
        metro.groupby("StateName")[metro_cols]
        .median()
        .reset_index()
...
# Build a best-available rent estimate using ZIP, metro, then Census fallback.
def select_best_rent_estimate(master: pd.DataFrame) -> pd.DataFrame:
    ...
    census_rent = (
        master["zori_rent"].isna()
        & master["zori_metro_rent"].isna()
        & master["median_gross_rent"].notna()
    ).sum()
...
# Build a best-available home value estimate from price then feature fallbacks.
def select_best_value_estimate(master: pd.DataFrame) -> pd.DataFrame:
...
# Compute NOI, cap rate, cash flow, and return metrics across down-payment scenarios.
def compute_cash_flow_metrics(master: pd.DataFrame) -> pd.DataFrame:
    ...
    master["effective_gross_income"] = master["annual_gross_rent"] - master["vacancy_loss"]
    ...
    master["total_operating_expenses"] = master["annual_maintenance"] + master["annual_mgmt_fee"]

    # ── Net Operating Income (NOI) ────────────────────────────────────────────────
    master["noi"] = master["effective_gross_income"] - master["total_operating_expenses"]
    ...
    for pct in [5, 10, 20]:
        ...
        if cash_flow_col in master.columns:
            ...
            master[f"coc_return_{pct}pct"] = (master[cash_flow_col] / down_payment).round(4)

    # ── Gross Rent Yield ──────────────────────────────────────────────────────────
    master["gross_rent_yield_calc"] = (master["annual_gross_rent"] / master["price"]).round(4)
    ...
    print_step(
        "Cash flow (20% down) computed:",
        f"{master['annual_cash_flow_20pct'].notna().sum():,} properties",
    )
    print_step(
        "CoC return (20% down) computed:",
        f"{master['coc_return_20pct'].notna().sum():,} properties",
    )
    ...
    for col, label in [
        ...
        if col in full.columns:
            mean_val = full[col].mean()
            median_val = full[col].median()
            std_val = full[col].std()
            print(f"  {label:<30} {mean_val:>10.3f} {median_val:>10.3f} {std_val:>10.3f}")
    return master


# Create binary cash-flow-positive labels for 20%, 10%, and 5% down scenarios.
def create_classification_labels(master: pd.DataFrame) -> pd.DataFrame:
    print_section("STEP 8: Creating Classification Label")

    master["is_cash_flow_positive"] = (master["annual_cash_flow_20pct"] > 0).astype("Int8")

    for pct in [5, 10]:
        col = f"annual_cash_flow_{pct}pct"
        if col in master.columns:
            master[f"is_cash_flow_positive_{pct}pct"] = (master[col] > 0).astype("Int8")
...
# Reorder columns into logical groups and print per-group column counts.
def organize_final_columns(master: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    derived_cols = [
        ...
        "noi",
        "cap_rate",
        "gross_rent_yield_calc",
        "annual_total_pmt_5pct",
        "annual_cash_flow_5pct",
        "coc_return_5pct",
        "annual_total_pmt_10pct",
        "annual_cash_flow_10pct",
        "coc_return_10pct",
        "annual_total_pmt_20pct",
        "annual_cash_flow_20pct",
        "coc_return_20pct",
    ]
    label_cols = [
...
# Print missing-value coverage for selected columns grouped by feature family.
def print_missing_value_summary(master: pd.DataFrame, groups: dict) -> None:
...
# Add final property-level investment ratios used for downstream analysis.
def add_property_ratios(master: pd.DataFrame) -> pd.DataFrame:
...
# Write the completed master table to disk.
def save_output(master: pd.DataFrame) -> None:
    out_path = os.path.join(OUTPUT_DIR, "master.csv")
    master.to_csv(out_path, index=False)


# Print the final completion summary and model-readiness note.
def print_completion(master: pd.DataFrame) -> None:
    print()
    print("=" * 60)
    print("  MASTER PREPROCESSING COMPLETE")
    print("=" * 60)
    print()
    print("  Output: outputData/master.csv")
...
# Run the full master preprocessing pipeline in the original sequence.
def main() -> None:
    realtor, zillow_zip, census_zip, metro = load_input_tables()
    realtor, zillow_zip, census_zip = normalize_zip_codes(realtor, zillow_zip, census_zip)
    master, before = join_zillow_zip_features(realtor, zillow_zip)
    master = join_census_zip_features(master, census_zip, before)
    master = join_metro_features(master, metro, before)
    master = select_best_rent_estimate(master)
    master = select_best_value_estimate(master)
    master = compute_cash_flow_metrics(master)
    master = create_classification_labels(master)
    master, groups = organize_final_columns(master)
    print_missing_value_summary(master, groups)
    master = add_property_ratios(master)
    save_output(master)
    print_completion(master)
