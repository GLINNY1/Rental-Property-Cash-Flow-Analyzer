...
def load_market_data():
    ...
    census_df["zip_code"] = census_df["zip_code"].astype(int).astype(str).str.zfill(5)
    print("done ✓")
...
def lookup_market_features(zip_code, zip_df, metro_df, census_df):
    ...
    # Mark whether we found real ZIP data
    features["_zip_found"] = zip_row is not None
...
def print_report(price, beds, baths, sqft, zip_code,
    ...
    # ── Rent Explanation ──────────────────────────────────────────────────────
    print()
    rent_src = market_features.get("_rent_source", "Census rent")
    print(f"  Rent Note: Base {rent_src} adjusted for {beds}BR using")
...
def main():
    ...
    while True:
        ...
        # Priority: ZIP-level ZORI (most accurate) > Census median rent > national median
        zori_rent = market_features.get("zori_rent")
        census_rent = market_features.get("median_gross_rent")
        if zori_rent and not pd.isna(zori_rent):
            base_rent = zori_rent
            rent_source = "Zillow ZORI (ZIP-level)"
        elif census_rent and not pd.isna(census_rent):
            base_rent = census_rent
            rent_source = "Census median rent"
        else:
            base_rent = census_df["median_gross_rent"].median()
            rent_source = "National median (fallback)"
        monthly_rent = adjust_rent_for_bedrooms(base_rent, beds, zip_row)
        market_features["_rent_source"] = rent_source
